...
//
        ...
        func `Linear interpolation instance method`() {
            ...
            let t: Scale<1, Double> = 0.5
            let mid = p1.lerp(to: p2, t: t)
            #expect(mid.x == 5)
            #expect(mid.y == 10)
        }
//
...
//
    ...
    struct `Map` {
        ...
        func `Map transforms each coordinate`() throws {
            let p = Point2(x: 3, y: 4)
            let doubled: Affine.Continuous<Int, Void>.Point<2> = p.map { Int($0 * 2) }
            #expect(doubled[0] == 6)
            #expect(doubled[1] == 8)
        }
//
        ...
        func `Map via init`() throws {
            let p = Point2(x: 3, y: 4)
            let doubled = Affine.Continuous<Int, Void>.Point<2>(p) { Int($0 * 2) }
            #expect(doubled[0] == 6)
            #expect(doubled[1] == 8)
        }
    }
